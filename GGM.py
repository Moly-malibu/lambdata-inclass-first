...
if __name__ == "__main__":
    ...
    my_frame = MyFrame({"abbrevs": ["EPS", "DPR", "RR", "ROE", "DIV0", "g", "DIV1", "PV"]})
    print(my_frame.head())
...
class dividend(share):
    ...
    def value_stock(self):
        return f"{self.EPS:,.0f} {self.DPR:,.0f}"    
    ...
    def retention_rate(self):
        return f"{self.DPR:,.0f} {self.RR:,.0f}"    
    def payout_ratio(self):
    ...
    def growth_rate(self):
        return f"{self.ROE:,.0f} {self.RR:,.0f}"  
    def growth_rate(self):
    ...
    def __init__(self, Dividend_per_share, growth_rate):
        ...
        self.g = g    @property
    def DIV0(self):
        return f"{self.DIV0:,.0f} {self.g:,.0f}"    
    def dividend_per_share_at_t_1(self):
    ...
    def DIV1(self):
        return f"{self.DIV1:,.0f} {self.rate_return:,.0f} {self.growth_rate} {self.pv}"  
    ...
    def Value_Company_stock():
        print(f"DIVIDEND VALUE!") 

    class PV(share):
        def __init__(self, earnings, ratio, PV):
            super().__init__(earnings, ratio)
        self.starting_center = PV    
    def PV(self):
        print(f"CALCULATION DIVIDEND PER SHARE & PRESENT VALUE {self.earnings.upper()} Company's Dividend!") @staticmethod
        print(f"Present Value!")
    
    if __name__ == "__main__": 
        PV = DIV1/(rCE-Growth_rate)   
